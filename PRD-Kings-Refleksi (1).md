...
### 5.7 Kontak & Lokasi
- Email: info@thekingsreflexology.com *(diupdate — domain final yang di-checkout klien adalah `thekingsreflexology.com`, bukan `kingsrefleksi.com` yang dipakai sebagai placeholder sebelumnya)*
...
Section anchor baru (`#promo`) berisi promo aktif yang di-update **mingguan atau lebih cepat**, berfungsi sebagai traffic funnel dari media sosial (Instagram/WA Story/dst. mengarahkan follower ke `thekingsreflexology.com/#promo`) — bukan URL terpisah, cukup anchor di homepage yang sudah ada.

**Struktur konten per promo:**
- Judul promo (misal "Promo Akhir Pekan", "Diskon Member Baru")
- Deskripsi singkat 1-2 kalimat
- Periode berlaku (tanggal mulai — tanggal akhir, atau "Selama kuota tersedia")
- CTA: arahkan ke WA cabang (pilih cabang mana — atau tampilkan semua 3 cabang seperti section Kontak)
- Opsional: foto/visual promo

**Keputusan arsitektur (karena update mingguan, situs tetap static tanpa CMS/DB):**
- Simpan sebagai data terstruktur di `data/promos.ts` — pola yang sama seperti `branches.ts`/`services.ts` yang sudah ditetapkan di Section 7
- Update mingguan = edit array di file ini + deploy ulang via Vercel (build & deploy Next.js biasanya selesai dalam hitungan detik-menit, jadi update cepat tetap memungkinkan tanpa CMS)
- Kalau ke depan **klien sendiri** (bukan developer) perlu update promo tanpa minta bantuan tiap minggu, ini titik keputusan yang sama seperti fitur reservasi (lihat catatan sebelumnya) — pertimbangkan CMS ringan (Notion-as-CMS/Airtable) HANYA kalau kebutuhan itu muncul, bukan dibangun di awal
- Kalau tidak ada promo aktif (array kosong), section ini sebaiknya di-hide otomatis dari halaman — bukan menampilkan "Belum ada promo" yang terkesan kosong/tidak terurus

**Navbar:** tambahkan item baru **"Promo"** di antara Member dan Tentang Kami (traffic dari sosmed butuh entry point yang jelas terlihat, bukan tersembunyi di tengah scroll) — navbar final jadi: `Home | Layanan | Member | Promo | Tentang Kami | Kontak`
...
| FR-7 | Section Promo (`#promo`) otomatis hilang dari halaman & navbar kalau array `data/promos.ts` kosong — tidak menampilkan section kosong/placeholder | Must |
...
| Konten update | Alamat, nomor WA, dan harga (kalau ditampilkan) kemungkinan berubah — simpan sebagai config/data file terpisah (`data/branches.ts`, `data/services.ts`, `data/promos.ts`), bukan hardcoded di JSX, agar update tidak perlu sentuh komponen. Khusus `promos.ts`: update mingguan atau lebih cepat, jadi struktur data harus benar-benar sederhana (array of object) supaya edit-deploy cepat dilakukan |
