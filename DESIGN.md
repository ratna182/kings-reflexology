...
colors:
  ...
  primary: '#12100e'                # Espresso — text, structural lines, primary buttons
  on-primary: '#ffffff'
  primary-container: '#30321c'
  on-primary-container: '#d6d7b8'   # brightened from #a8a6a5 to clear 4.5:1 on primary-container
  inverse-primary: '#c2bd9f'

  secondary: '#4a4b2f'              # Olive — secondary text, metadata, disabled states
  on-secondary: '#ffffff'
  secondary-container: '#e3dfd0'
  on-secondary-container: '#4a4b2f'

  tertiary: '#6b654b'               # Olive-Tan — reserved exclusively for primary CTA / status
  on-tertiary: '#ffffff'
  tertiary-container: '#30321c'
  on-tertiary-container: '#e0dcbf'
  ...
  primary-fixed: '#e3dfd0'
  primary-fixed-dim: '#c2bd9f'
  on-primary-fixed: '#12100e'
  on-primary-fixed-variant: '#3d3e2c'
  secondary-fixed: '#e3dfd0'
  secondary-fixed-dim: '#c3bea6'
  on-secondary-fixed: '#12100e'
  on-secondary-fixed-variant: '#3d3e2c'
  tertiary-fixed: '#e0dcbf'
  tertiary-fixed-dim: '#c2bd9f'
  on-tertiary-fixed: '#12100e'
  on-tertiary-fixed-variant: '#3d3e2c'

  background: '#fcf9f5'
  on-background: '#12100e'
  surface-variant: '#e3dfd0'

  # --- Accessibility: focus state (previously missing) ---
  focus-ring: '#6b654b'             # reuses Tertiary — keeps focus state on-brand, not generic blue
  focus-ring-offset: '#fcf9f5'      # matches background so ring reads as a clean halo

  # --- Disabled state (previously undefined) ---
  disabled-fill: '#e3dfd0'
  disabled-on: '#8a8568'            # ~3:1 against disabled-fill — intentionally muted, non-interactive signal
...
## Colors
The palette is rooted in organic, botanical tones. **The tokens below are the single source of truth** — the YAML front-matter has been reconciled to match, so implementers should map directly to the named tokens (`primary`, `secondary`, `tertiary`) rather than pulling from unrelated M3-generated fields.

- **Primary — `#12100E` (Espresso):** Used for primary text, structural lines, and solid primary buttons. It provides the "ink" on the page.
- **Secondary — `#4A4B2F` (Olive):** Desaturated green for secondary text, metadata, and disabled states.
- **Tertiary — `#6B654B` (Olive-Tan):** A muted earthy accent reserved for **primary conversion actions** — see the revised CTA hierarchy rule below. Never use it decoratively — its rarity is what gives it weight.
...
- **Deep Olive — `#30321C`:** The dark tonal accent — used for containers, inverse surfaces, and dark-mode surfaces. Pairs with `#12100E` to keep the green botanical read without noise.

The palette was translated from the warm-brown original to an olive-green "botanical" family (`#12100E / #30321C / #4A4B2F / #6B654B`) that speaks to the reflexology & wellness brand. Two tokens were adjusted for accessibility:
- `on-primary-container` is `#D6D7B8`, bright enough to clear 4.5:1 contrast against `primary-container` (`#30321C`).
- `disabled-on` was muted to `#8A8568` so disabled states read unambiguously as non-interactive.
...
- **Borders:** Use 1px solid lines in Primary (`#12100E`) at low opacity (10–20%) to separate sections without creating visual noise.
...
- **Buttons:**
  - *Primary:* Solid Espresso (`primary`) with white text (`on-primary`). Sharp corners.
  - *CTA (Olive-Tan accent):* Solid Olive-Tan (`tertiary`) with white text (`on-tertiary`).
    **Revised rule (per PRD v1.1):** the original one-CTA-per-site rule assumed a single "Book Now" action. The PRD now has multiple WhatsApp CTAs (Hero, Member section, and 3 branch-specific contacts) — an entire site with one Olive-Tan button total is no longer realistic. Revised scope: **one Olive-Tan CTA per section/viewport**, not per site. Concretely — the Hero's main action, the Member section's main action, and *one* branch's WA button at a time (e.g. the closest/featured branch, or whichever the user has scrolled to) may use Olive-Tan; every other WA link on screen at once falls back to *Secondary* (outlined). This keeps Olive-Tan functioning as "the one thing to click here" within any given view, preserving its weight without contradicting the multi-CTA reality of the PRD.
  - *Secondary:* Outlined (1px, `primary`), no fill. Default style for WA links in the Kontak/Lokasi grid (3 branches side by side) — using Olive-Tan on all 3 simultaneously would flatten the accent's meaning.
...
- **Navigation:** Minimalist top-bar with `label-caps` links. At most one nav-adjacent element (e.g. a single "Kontak" or featured WA link) may use the Tertiary (`#6B654B`) accent at a time — see the revised CTA hierarchy rule under Buttons.
